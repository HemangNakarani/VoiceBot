...
export default function setChannels(calendarChannels:Array<valueInterface>) {
  let button = document.querySelector('[data-testid="socialChannelTypes-filter-summary"]') as HTMLElement;
  button.click();

  setFilter(calendarChannels).then(async () => {
    button = await checkElement('[data-testid="socialChannelTypes-filter-summary"]');
    button.click();
  });
}
